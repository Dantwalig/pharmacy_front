...
export default function StaffInventoryPage() {
  ...
  const fetchMedications = async () => {
    ...
    try {
      ...
      // When the backend team adds Role.PHARMACIST to this endpoint's @Roles decorator,
      ...
      if (err?.response?.status === 403) {
        // 403 means the backend role guard is blocking access — feature pending backend update
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-3 px-4 py-4 rounded-xl border border-yellow-200 bg-yellow-50">
          ...
          <div>
            ...
            <p className="text-xs text-yellow-700 mt-0.5">
              Inventory management for pharmacists is built and ready. The backend team needs to add
              ...
              endpoints. No frontend changes will be needed once that is done.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl p-4 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between border border-gray-100">
        ...
        <button
          onClick={() => router.push('/staff/inventory/add')}
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map((med: any) => (
                  <tr key={med.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <button
                        onClick={() => router.push(`/staff/inventory/${med.id}`)}
