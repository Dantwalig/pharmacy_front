...
export default function StaffAddMedicationPage() {
  ...
  useEffect(() => {
    // Fetch the staff member's branch — staff belong to one branch so we pre-select it
    // GET /staff/profile/me returns branch info
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.branchId) { toast.error('Branch not found'); return; }
    setLoading(true);
    try {
      // BACKEND PENDING: POST /medications
      // This endpoint currently restricts access to Role.PHARMACY only.
      // When the backend team adds Role.PHARMACIST to the @Roles decorator on this endpoint,
      // this call will work automatically with no frontend changes needed.
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
        ...
        <div className="flex items-start gap-3 px-4 py-4 rounded-xl border border-yellow-200 bg-yellow-50">
          ...
          <div>
            ...
            <p className="text-xs text-yellow-700 mt-0.5">
              ...
              <span className="font-mono font-bold mx-1">Role.PHARMACIST</span>
              to the
              <span className="font-mono mx-1">POST /medications</span>
              endpoint. The form below is fully built and will work immediately once access is granted.
            </p>
          </div>
        </div>
      )}
      ...
      {branchesLoading ? (
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-6 border border-gray-100 space-y-5">
          ...
          {/* Branch (pre-selected, read-only) */}
          <div>
            <label className={labelCls}>Branch</label>
            <input
              type="text"
              readOnly
              value={branches[0]?.name ?? 'Loading...'}
