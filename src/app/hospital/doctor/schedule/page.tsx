...
function startHour(entry: { startTime?: string }) {
    if (!entry.startTime) return -1;
    return parseInt(entry.startTime.split(':')[0], 10);
}
                ...
                {days.map((day) => {
                    ...
                    return (
                        <button
                            ...
                            className={`w-6 h-6 mx-auto rounded-full flex items-center justify-center transition-colors ${sel
...
export default function DoctorSchedulePage() {
    ...
    const fetchSlotsForDate = useCallback(async (d: Date) => {
        if (!doctorId) return;
        const dStr = dayKey(d);
        try {
            const res = await api.get(`/doctors/${doctorId}/slots?date=${dStr}`);
            const data = unwrapData<any>(res.data) || [];
            // Backend returns slots, wrap it in array if needed or extract items
            const arrayData = Array.isArray(data) ? data : [];
            setSlotsByDate(prev => ({ ...prev, [dStr]: arrayData }));
        } catch (err) {
            console.error("Failed to load slots for " + dStr);
        }
    }, [doctorId]);

    // Reactively fetch slots for currentDate, and if in Calendar mode, fetch for the whole week
    useEffect(() => {
        if (view === 'LIST') {
            fetchSlotsForDate(currentDate);
        } else {
            weekDays.forEach(d => fetchSlotsForDate(d));
        }
    }, [currentDate, view, fetchSlotsForDate]); // intentional dependencies

    const entriesForDay = (d: Date) => slotsByDate[dayKey(d)] || [];
    const entryAt = (d: Date, hour: number) =>
        entriesForDay(d).find((e) => startHour(e) === hour);
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-xl">
                    <button
                        ...
                        className={`px-6 py-2 rounded-lg text-sm font-semibold transition-all ${view === 'LIST' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-800'
                            }`}
                    >
                        Daily Schedule
                    </button>
                    <button
                        ...
                        className={`px-6 py-2 rounded-lg text-sm font-semibold transition-all ${view === 'CALENDAR' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-800'
                            }`}
                    >
                        Weekly Overview
                    </button>
            ...
            {view === 'LIST' && (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 sm:p-4 space-y-2">
                    {HOURS.map((hour) => {
                        ...
                        if (entry) {
                            const accent = COLOR_MAP[entry.color as EntryColor] ?? COLOR_MAP.blue;
                            return (
                                <div
                                    key={hour}
                                ...
                                >
                                    ...
                                    <span className="text-sm font-bold text-gray-900 flex-1 truncate">{entry.patientName || 'Reserved Slot'}</span>
                    ...
                    <aside className="w-full lg:w-64 shrink-0 bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-5">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 pl-1">
                                ...
                                <span className="text-xs text-gray-600 flex-1 truncate">Doctor</span>
                                ...
                                {weekDays.map((day) => {
                                    ...
                                    return (
                                        <div key={day.toISOString()} className="py-3 text-center border-l border-gray-100">
                                            ...
                                            <p
                                                className={`mt-1 text-sm font-bold inline-flex items-center justify-center w-7 h-7 rounded-full ${todayCol ? 'bg-blue-600 text-white' : 'text-gray-700'
                                                    }`}
                                            >
                                                {format(day, 'd')}
                                            </p>
                                                ...
                                                {entry && (() => {
                                                    ...
                                                    return (
                                                        <div className={`h-full rounded-lg border p-2 ${accent}`}>
                                                            ...
                                                            <p className="text-xs font-bold leading-tight mt-0.5 truncate">{entry.patientName || 'Reserved'}</p>
